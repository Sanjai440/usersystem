...
//         // ✅ FIXED LINE (ONLY BACKEND)
//         const finalStatus = data.trainingCompleted;

//         setTrainingDone(finalStatus);

//         // optional cache (NOT source of truth)
//         localStorage.setItem("trainingCompleted", finalStatus);
...
//         // ✅ ONLY FIX (STRICT BOOLEAN CONVERSION)
//         setTrainingDone(!!data.trainingCompleted);
...
function Dashboard() {
  ...
  // 🔥 fetch backend status (SOURCE OF TRUTH)
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const res = await fetch(
        ...
        // ✅ FIX: combine backend + localStorage sync
        const completed =
          data.trainingCompleted ||
          localStorage.getItem("trainingCompleted") === "true";
