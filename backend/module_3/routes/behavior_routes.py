...
def receive_behavior():
    try:
        ...
        # 🟢 detect type (training / exam)
        data_type = data.get("type", "training")
        ...
        # 🟡 separate storage for training vs exam
        if data_type == "exam":
            keyboard_path = os.path.join(keyboard_dir, "exam_key_events.json")
            mouse_path = os.path.join(mouse_dir, "exam_mouse_events.json")
        else:
            keyboard_path = os.path.join(keyboard_dir, "key_events.json")
            mouse_path = os.path.join(mouse_dir, "mouse_events.json")
        ...
        # ⚙️ feature extraction (same for both training & exam)
        features = extract_features(events, user_id=user_id)
